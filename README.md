...
## `python -m shbuild <project>` — standalone `.sh` builder

A separate, zero-CLI-dependency tool in this repo: hand it a Python project directory and it generates one self-contained executable `<name>.sh` (a bash script with an embedded venv) that runs the project on a bare machine.

```bash
python -m shbuild <project_dir> [--output out.sh] [--main main.py]
```

- **Dependencies**: auto-detected from `requirements.txt` and/or `pyproject.toml`; the generated script installs them into its own venv on first run (and every run, in case the machine's Python moved).
- **Args passthrough**: everything after the project directory is forwarded to the built program at runtime.
- **Built-in checks**: verifies the target file exists and that dependencies are parseable before writing anything out — you get a clean error instead of a broken script.

The generated script is what `test/test_e2e_sh.py` exercises end-to-end (it builds real sample projects, runs them in near-clean environments, and asserts on output/exit codes). This is the recommended way to hand JFI itself to someone who doesn't want to install Python tooling first.
...
├── pyproject.toml               # package metadata, deps (ruff/pytest for dev), pytest config
...
│
├── src/shbuild/                 # standalone .sh builder (separate from JFI's agent code)
│   ├── __main__.py              # CLI entry point: python -m shbuild <project> [--output] [--main] [passthrough args…]
│   └── builder.py               # dependency detection + template rendering into a self-contained bash script
│
├── test/                        # pytest suite (see "Tests" below) — unit tests per module plus e2e for shbuild and the plan-file protocol
└── docs/
...
- `test_review_outcome.py` / `test_payload_roundtrip.py` — the review-loop decision logic and message-shape round-tripping.
- `test_e2e_sh.py` — real end-to-end runs of shbuild-generated scripts in near-clean environments.
